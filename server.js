...
const app = express();
app.use(cors({
    origin: ['http://localhost:5173', 'http://localhost:4173', 'https://agency-hub-ny.netlify.app', /\.netlify\.app$/],
...
function deductTokens(brand, operation, details = '') {
    ...
    if (!credits[brand]) credits[brand] = {
        balance: 0,
        plan: 'starter',
        transactions: []
    };
...
app.post('/api/tokens/add', (req, res) => {
    ...
    if (!brand || !tokens) return res.status(400).json({
        error: 'brand y tokens son requeridos'
    });
    const credits = loadCredits();
    if (!credits[brand]) credits[brand] = {
        balance: 0,
...
async function transcribeAudio(audioBuffer) {
    try {
        const file = await toFile(audioBuffer, 'video.mp4', {
            type: 'audio/mp4'
        });
...
app.post('/api/viral-cloner', async (req, res) => {
    ...
    try {
        ...
        const brandContext = brand === 'lacancha' ? 'La Cancha: Sede de la seleccion colombia en NY, eventos, tejo, rumba. Colombianos/latinos en NY.' : 'S.Meat & Hueso: Restaurante Asador NY. All you can eat, fuego, lena. Foodies en NY.';
        const guionContext = realTranscript ? `TRANSCRIPCION REAL (Whisper):\n${realTranscript}` : `METADATA - Titulo: ${videoData.title}`;
...
try {
    ...
    const brandContext = brand === 'lacancha' ? 'La Cancha: Sede de la seleccion colombia en NY, eventos, tejo, rumba. Colombianos/latinos en NY.' : 'S.Meat & Hueso: Restaurante Asador NY. All you can eat, fuego, lena. Foodies en NY.';
    const guionContext = realTranscript ? `TRANSCRIPCION REAL (Whisper):\n${realTranscript}` : `METADATA - Titulo: ${videoData.title}`;
    const systemPrompt = `Eres el mejor Clonador Viral del mundo. Transcriptor Forense + Director Creativo. FORMULA DE PUNTUACION (OBLIGATORIA): - Like/View (40pts): >5%=40, >2%=28, >0.5%=16, <0.5%=8 - Comment (30pts): ratio >8%=30, >4%=22, >1%=14, <1%=6 - Share (30pts): coeff >5=30, >2=22, >0.5=12, <0.5=5 Responde UNICAMENTE JSON: { "originalScript": "Guion completo", "clonedScript": "Clon completo", "psychology": "Tecnica viral", "viralScore": 0, "scoringBreakdown": { "likeViewScore": "XX/40", "commentScore": "XX/30", "shareScore": "XX/30" }, "predictiveFeedback": [], "shotList": [], "storyboard": [], "copySEO": { "caption": "", "hashtags": [] } }`;
    const completion = await openai.chat.completions.create({
        model: 'gpt-4o',
        temperature: 0.55,
        response_format: {
        ...
        messages: [{
            ...
            role: 'user',
            content: `Marca: ${brandContext}\nMetricas: Views=${views}, Likes=${likes}\n${guionContext}`
        }]
    });
    const aiPayload = JSON.parse(completion.choices[0].message.content);
    res.json({
...
try {
    ...
    const brandContext = brand === 'lacancha' ? 'La Cancha: Sede de la seleccion colombia en NY, eventos, tejo, rumba.' : 'S.Meat & Hueso: Restaurante Asador NY. Foodies en NY.';
    const guionContext = realTranscript ? `TRANSCRIPCION REAL:\n${realTranscript}` : `METADATA - Titulo: ${videoData.title}`;
    const systemPrompt = `Eres un Clonador Viral. Responde JSON: { "originalScript": "", "clonedScript": "", "psychology": "", "viralScore": 0, "scoringBreakdown": {}, "predictiveFeedback": [], "shotList": [], "storyboard": [], "copySEO": { "caption": "", "hashtags": [] } }`;
    const completion = await openai.chat.completions.create({
        model: 'gpt-4o',
        temperature: 0.55,
        response_format: {
...
app.post('/api/strategy', async (req, res) => {
    ...
    try {
        ...
        const completion = await openai.chat.completions.create({
            ...
            messages: [{
                role: 'system',
                content: 'Matriz de Contenido. JSON: { "funnelDistribution": {}, "strategy": [] }'
            }, {
                role: 'user',
                content: `Marca: ${brandContext}\nEvento: ${eventType}`
            }]
...
app.post('/api/flyer-copy', async (req, res) => {
    ...
    try {
        ...
        const completion = await openai.chat.completions.create({
            ...
            messages: [{
                role: 'system',
                content: 'Copywriter. JSON: { "topDate": "", "cursiveHeadline": "", "mainArtist": "", "prices": {}, "callToAction": "", "promptImagen": "" }'
            }, {
                role: 'user',
                content: `Marca: ${brandContext}\nEvento: ${eventType}`
            }]
